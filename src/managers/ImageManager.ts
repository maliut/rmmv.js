...
export class ImageManager {
  ...
  static loadBitmap(folder: string, filename: string, hue = 0, smooth = false) {
    if (filename) {
  ...
  static reserveBitmap(folder: string, filename: string, hue = 0, smooth = false, reservationId = this._defaultReservationId) {
    if (filename) {
      const path = folder + encodeURIComponent(filename) + '.png'
      const bitmap = this.reserveNormalBitmap(path, hue, reservationId)
      bitmap.smooth = smooth
      return bitmap
    } else {
      return this.loadEmptyBitmap()
  ...
  static requestBitmap(folder: string, filename: string, hue = 0, smooth = false) {
    if (filename) {
      const path = folder + encodeURIComponent(filename) + '.png'
      const bitmap = this.requestNormalBitmap(path, hue)
      bitmap.smooth = smooth
      return bitmap
    } else {
      return this.loadEmptyBitmap()
